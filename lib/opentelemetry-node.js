...
const {
  ...
  ATTR_URL_PATH, ATTR_SERVER_ADDRESS,
  ATTR_SERVER_PORT, ATTR_URL_SCHEME,
...
/**
 * Message property carrying the run identifier. Kept under its historical name so that flows and dashboards referring to it keep working;
...
/**
 * What each node is currently processing, used to attribute a newly created message to the run its emitting node belongs to
 * (Node-RED mints a fresh `_msgid` for every message object a node emits that does not already have one).
...
/**
 * Find the key of an already open span for this node, tolerating a node that emitted a new message (and therefore a new `_msgid`) while it was processing another one
...
/**
 * Start a new run: one root span standing for this flow execution, parented to the caller context when the entry node carries one,
 * and linked to the run it continues when the message comes from an execution that already finished (an asynchronous hand-off).
...
function createRun (tracer, msg, nodeDefinition, predecessorRunId) {
  // a run is identified by the message that started it, kept unique so that a message sent again later cannot be mistaken for the run it originally belonged to
  ...
  const flowName = _hasFlowNameInRoot ? nodeDefinition._flow?.flow?.label + ' ' ?? '' : ''
  const rootSpan = tracer.startSpan(_rootPrefix + flowName + (nodeDefinition.name || nodeDefinition.type), {
    attributes: {
      ...
      // what set this run off, kept on the run span so a collector can route or filter on it without looking at the node spans below
...
/**
 * Close runs left behind by nodes that never report completion. Node spans still open are ended (and flagged) instead of being dropped, so the trace keeps them.
 */
function sweepRuns () {
  const now = Date.now()
  try {
...
function createSpan (tracer, msg, nodeDefinition, candidateRunIds, isNotTraced, isEntry, sourceNodeId) {
  try {
    ...
    // Carry the run on the message, so every node downstream joins the same trace even when a node emits a brand new message object (Node-RED gives those a fresh `_msgid`). This also overwrites the run of a message emitted again long after its original run finished
    setRunId(msg, run.id)
...
/**
 * Build the headers the exporter has to send, from the configured authentication and the additional headers some collectors require (dataset, organization, stream, ...).
 *
 * Secrets live in the node credentials, never in the flow configuration, so they stay out of `flows.json` and out of any exported flow. Headers set in
 * `OTEL_EXPORTER_OTLP_HEADERS` are merged by the SDK itself; a header defined here wins over the environment for the same key.
 *
...
module.exports = function (RED) {
  ...
  function OpenTelemetryNode (config) {
    ...
    // The `.otel` hook label is global to the Node-RED runtime, so a single node can own the hooks. A second one stays inactive rather than failing the deploy with "Hook onSend.otel already registered", and leaves the active node's settings alone
    ...
    RED.hooks.add('onSend.otel', (events) => {
      ...
      // a node emitting a message without processing one starts a run and never reports completion, so its span is flagged to be closed on dispatch
    ...
    RED.hooks.add('preDeliver.otel', (sendEvent) => {
      // the run is already carried by the message: `onSend` stamped it before the message was cloned for delivery, so every clone reaching this point belongs to it
